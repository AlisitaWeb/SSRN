...
def main():
    ...
    parser.add_argument('--in-path', type=str, default=r'D:\PT\archive\Testingset10class\dataset\结果\test_128',
                        help='image to test')
    parser.add_argument('--out-path', type=str, default=r'D:\PT\archive\Testingset10class\dataset\结果\dsrl\128_1', help='mask image to save')
    parser.add_argument('--backbone', type=str, default='mobilenet',
                        choices=['resnet', 'xception', 'drn', 'mobilenet'],
                        help='backbone name (default: resnet)')
    parser.add_argument('--ckpt', type=str, default=r'D:\PT\超分辨率语义分割\模型保存\1\dsrl\128/model_best.pth.tar',
                        help='saved model')
    ...
    for name in os.listdir(args.in_path):
        ...
        if args.cuda:
            ...
            '''
            tensor_in = torch.nn.functional.interpolate(tensor_in, size=[i // 2 for i in tensor_in.size()[2:]], mode='bilinear',
                                                        align_corners=True)
            '''
        with torch.no_grad():
            output,_,_,_ = model(tensor_in)
